...
class ResetDetector:
    ...
    def __init__(self, config):
        ...
        self.cache["object_status"] = {obj: True for obj in self.objects}
    ...
    def detect(self, image: np.ndarray):
        ...
        if self.config["reset_detector_params"]["which_vlm"] != "none":
            ...
            for object in self.objects:
                image_prompt = (
                    self.prompt_components[0]
                    + self.prepend_article(object)
                    + self.prompt_components[1]
                )
            ...
            if self.config["reset_detector_params"]["which_vlm"] == "cogvlm":
                vlm_output = utils.ask_cogvlm_batched(
                    images, image_prompts, self.config
                )
            elif self.config["reset_detector_params"]["which_vlm"] == "gpt4v":
                vlm_output = utils.ask_gpt4v_batched(images, image_prompts)
            else:
                vlm_output = (
                    "Yes, " + self.prepend_article(object) + " is present in the image."
                )
            ...
            updated_object_status = {obj: True for obj in self.objects}
        ...
        missing_objects = [
            obj for obj in objects_not_present if self.cache["object_status"][obj]
        ]
        ...
        to_return = {}
        for i in range(len(objects_present)):
            to_return[self.objects[i]] = objects_present[
                i
            ]  # we're commenting this out bc the VLM can be unreliable, and we don't want to not propose tasks we can actually execute
        return to_return

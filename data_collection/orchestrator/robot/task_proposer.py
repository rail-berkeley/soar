...
class ProposerFromTaskList(AbstractTaskProposer):
    ...
    def load_possible_tasks(self, config):
        ...
        if "task_list" in config["task_definition_params"]:
            ...
            print(
                "Enter a list of possible tasks that the robot can perform in this environment (press enter to exit)"
            )
            counter = 1
            while True:
        ...
        self.possible_task_to_id = {
            task: i for i, task in enumerate(self.possible_tasks)
        }
        self.id_to_possible_task = {
            i: task for i, task in enumerate(self.possible_tasks)
        }
...
class VLMTaskProposer(ProposerWithCounters):
    ...
    def get_viable_tasks(self, image_obs: np.ndarray, off_limits_objects=None):
        ...
        task_to_vqa_prompts = [
            self.task_to_vqa_prompt + task for task in filtered_task_list
        ]
        ...
        vlm_prompts = [
            vqa_prompts_parsed[i]["vlm_question"]
            for i in range(len(vqa_prompts_parsed))
        ]
        ...
        decoding_prompts = [
            ...
            for i in range(len(vlm_answers))
        ]
        ...
        vlm_answers_decoded = [
            vlm_answer.strip().lower() for vlm_answer in vlm_answers_decoded
        ]
        task_viable_answers = [
            vqa_prompt["answer"].strip().lower() for vqa_prompt in vqa_prompts_parsed
        ]
        viable_tasks_bool = (
            np.array(task_viable_answers) == np.array(vlm_answers_decoded)
        ).tolist()
        viable_task_ids = []
        for i, task in enumerate(filtered_task_list):
...
class HistoryAwareVLMTaskProposer(VLMTaskProposer):
    ...
    def propose_task(self, image_obs: np.ndarray, off_limits_objects=None):
        ...
        for task_id in task_ids:
            score = np.sqrt(
                np.log(total_task_executions + 1)
                / (self.task_attempted_counter[task_id] + 1)
            )
            print(
                f'Task "{self.possible_tasks[task_id]}": UCB score: {score}, ', end=""
            )
            task_success_rate = (
                0.0
                if self.task_attempted_counter[task_id] == 0
                else self.task_success_counter[task_id]
                / self.task_attempted_counter[task_id]
            )

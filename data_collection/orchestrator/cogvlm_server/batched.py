...
model = (
    AutoModelForCausalLM.from_pretrained(
        MODEL_PATH, torch_dtype=torch_type, load_in_4bit=False, trust_remote_code=True
    )
    .to(DEVICE)
    .eval()
)
...
def query_model():
    ...
    for i, file in enumerate(image_files):
        ...
        input_by_model = model.build_conversation_input_ids(
            tokenizer, query=prompts[i], history=[], images=[image]
        )
        inputs = {
    ...
    for i in range(len(model_inputs)):
        ...
        model_inputs[i]["input_ids"] = torch.cat(
            [
                torch.zeros(
                    (1, max_len - tensor_shape), dtype=torch.long, device=DEVICE
                ),
                model_inputs[i]["input_ids"],
            ],
            dim=1,
        )
        model_inputs[i]["token_type_ids"] = torch.cat(
            [
                torch.zeros(
                    (1, max_len - tensor_shape), dtype=torch.long, device=DEVICE
                ),
                model_inputs[i]["token_type_ids"],
            ],
            dim=1,
        )
        model_inputs[i]["attention_mask"] = torch.cat(
            [
                torch.zeros(
                    (1, max_len - tensor_shape), dtype=torch.long, device=DEVICE
                ),
                model_inputs[i]["attention_mask"],
            ],
            dim=1,
        )
    ...
    combined_inputs = {
        ...
        "token_type_ids": torch.cat(
            [inputs["token_type_ids"] for inputs in model_inputs], dim=0
        ),
        "attention_mask": torch.cat(
            [inputs["attention_mask"] for inputs in model_inputs], dim=0
        ),
        "images": concatenated_images,
    }

    gen_kwargs = {"max_length": 2048, "temperature": 0.0, "do_sample": False}
